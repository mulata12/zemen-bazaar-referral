...
function UserRegistration() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const queryParam = formData.referralCode
        ? `?rCode=${encodeURIComponent(formData.referralCode)}`
        : "";

      const res = await fetch(
        `${BACKEND_URL}/auth/user/register${queryParam}`,
        {
          method: "POST",
          headers: {
