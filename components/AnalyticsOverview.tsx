...
export default function AnalyticsOverview() {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        ...
        // Map backend data to frontend analytics structure
        setAnalytics({
  ...
  const successRate = analytics.totalReferrals
    ? ((analytics.successful! / analytics.totalReferrals) * 100).toFixed(1)
    : '0';

  const monthlyMax = Math.max(...(analytics.monthlyTrend || [0]));
          ...
          {(analytics.monthlyTrend || []).map((value, index) => (
            <div key={index} className="flex-1 flex flex-col items-center">
              <div
                ...
                style={{ height: `${(value / (monthlyMax || 1)) * 80}%` }}
              ></div>
